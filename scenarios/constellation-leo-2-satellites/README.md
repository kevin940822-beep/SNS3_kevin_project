| 欄位 | 名稱 | 說明 | 範例 |
|---|---|---|---|
| 1 | Waveform Index | SNS3 內部的流水號 |`1`~`22`<br>且 `default_waveform.txt` : 預設 UT 會使用第 3 行的配置來起步)|
| 2 | DVB-RCS2 ID | 官方標準波形編號 |`2`~`22`|
| 3 | Modulation Order | 每個符號的位元數 |`2`=QPSK (最穩定、抗雜訊最強，但速度慢) <br> `3`=8PSK <br> `4`=16QAM(速度最快，但需要極高的訊號品質)|
